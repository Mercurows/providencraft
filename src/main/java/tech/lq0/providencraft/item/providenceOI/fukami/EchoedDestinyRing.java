...
public class EchoedDestinyRing extends Item {
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        tooltip.add(new TranslationTextComponent("des.providencraft.echoed_destiny_ring_1").mergeStyle(TextFormatting.GRAY).mergeStyle(TextFormatting.ITALIC));
        tooltip.add(new TranslationTextComponent("des.providencraft.echoed_destiny_ring_2").mergeStyle(TextFormatting.GRAY));
                ...
                for (LivingEntity livingentity : player.getEntityWorld().getEntitiesWithinAABB(LivingEntity.class, player.getBoundingBox().grow(30.0D, 30.0D, 30.0D))) {
                    if (livingentity instanceof PlayerEntity && livingentity != player && livingentity.isOnSameTeam(player)) {
                        ...
                        if(player.ticksExisted % 300 == 0){
                            playerEntity.addPotionEffect(new EffectInstance(Effects.REGENERATION, 300, 1, true, false));
                        }
                    }
                }
            } else {
